...
const baseline = path.join(project, 'backups/version-1.6');
...
const pilotNodes = afterNodes.nodes.filter(node => node.metadata?.enrichment?.version === '1.7');
const pilotRelations = afterRelations.relations.filter(relation => relation.id.startsWith('security-rel-'));
...
for (const [id, before] of beforeById) {
  ...
  if (after.metadata?.enrichment?.version !== '1.7') {
    ...
    if (stable(priorRefs) !== stable((after.relations || []).filter(rel => !rel.startsWith('security-rel-')))) fail(`${id}: Historische Relationsreferenzen verändert.`);
  }
}
