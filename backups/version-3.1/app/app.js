(function(){
  ...
  const dom={
    ...
    path:el('detailPath'),audit:el('auditBox'),simpleSection:el('simpleSection'),short:el('detailShort'),technicalSection:el('technicalSection'),technical:el('detailTechnical'),architectureSection:el('architectureSection'),architecture:el('detailArchitecture'),whySection:el('whySection'),why:el('detailWhy'),examplesSection:el('examplesSection'),examples:el('detailExamples'),memorySection:el('memorySection'),memory:el('detailMemory'),meta:el('detailMeta'),learningSection:el('learningSection'),learning:el('learningStatus'),
    ...
    actions:el('detailActions'),toggle:el('toggleBranch'),showRelations:el('showRelations'),back:el('backNavigation'),exportProfile:el('exportProfile'),importProfile:el('importProfile'),profileFile:el('profileFile')
  };
  const state={
  ...
  const releaseSummary=`V${RELEASE.release_version} Wissensbasis · V${NAV.meta?.release_version||'3.1'} Navigation · ${Number(RELEASE.node_count).toLocaleString('de-DE')} Knoten · ${Number(RELEASE.relation_count).toLocaleString('de-DE')} Beziehungen · ${RELEASE.scenario_count} Szenarien · ${RELEASE.learning_path_count} Lernpfade`;
  dom.summary.textContent=releaseSummary;
  document.title=`${RELEASE.product_name} V${NAV.meta?.release_version||'3.1'}`;
  ...
  function linkPresentation(match){
    ...
    return{type,targets,ids:classified.map(candidate=>candidate.nodeId)};
  }
  ...
  function setMode(mode){
    ...
    if(brain){buildGraph(state.graph.focusId);resizeBrain();fitGraph()}else if(mind){renderMindmap();if(state.selected)centerMindNode(state.selected);else fitMindmap()}else if(architecture)renderArchitecture();else renderLearning();
  }
  ...
  function buildGraph(focusId=null){
    state.graph.focusId=focusId||null;const include=new Set(),synthetic=new Map();
    if(focusId&&nodeById.has(focusId)){
      const n=nodeById.get(focusId);include.add(n.id);ancestors(n).forEach(a=>include.add(a.id));n.childNodes.forEach(c=>{include.add(c.id);c.childNodes.slice(0,8).forEach(g=>include.add(g.id))});
      for(const r of RELS)if(r.source===focusId||r.target===focusId){include.add(r.source);include.add(r.target)}
      for(const id of [...include])for(const r of RELS)if(r.source===id||r.target===id){if(include.size<120){include.add(r.source);include.add(r.target)}}
    }else{
    ...
    usedCategories.forEach(category=>synthetic.set(`domain:${category}`,{id:`domain:${category}`,title:category,category,type:'domain',r:17}));
    const graphNodes=[...synthetic.values(),...[...include].map(id=>nodeById.get(id)).filter(Boolean).map(n=>({id:n.id,title:n.title,category:n.category,type:'knowledge',r:Math.max(4,Math.min(12,3+importance(n)*.55)),importance:importance(n)}))];
    ...
    for(let iteration=0;iteration<80;iteration++){
      for(let i=0;i<graphNodes.length;i++)for(let j=i+1;j<graphNodes.length;j++){const a=graphNodes[i],b=graphNodes[j],dx=b.x-a.x,dy=b.y-a.y,d2=dx*dx+dy*dy+.1,min=(a.r+b.r+18),d=Math.sqrt(d2);if(d<min){const f=(min-d)/d*.035,fx=dx*f,fy=dy*f;if(a.type!=='domain'){a.x-=fx;a.y-=fy}if(b.type!=='domain'){b.x+=fx;b.y+=fy}}}
      for(const e of graphEdges.filter(e=>!e.synthetic)){const a=graphNodes.find(n=>n.id===e.source),b=graphNodes.find(n=>n.id===e.target);if(!a||!b)continue;const dx=b.x-a.x,dy=b.y-a.y,d=Math.sqrt(dx*dx+dy*dy)||1,f=(d-160)*.002;if(a.type!=='domain'){a.x+=dx/d*f;a.y+=dy/d*f}if(b.type!=='domain'){b.x-=dx/d*f;b.y-=dy/d*f}}
    }
    state.graph.nodes=graphNodes;state.graph.edges=graphEdges;dom.focus.textContent=focusId?'Fokus lösen':'Gesamtansicht';dom.visible.textContent=`${graphNodes.filter(n=>n.type==='knowledge').length} Wissensknoten · ${graphEdges.filter(e=>!e.synthetic).length} semantische Kanten`;drawGraph();
  }
  ...
  function drawGraph(){
    ...
    const selected=state.selected,direct=new Set();if(selected)for(const e of state.graph.edges)if(e.source===selected||e.target===selected){direct.add(e.source);direct.add(e.target)}
    for(const e of state.graph.edges){const a=state.graph.nodes.find(n=>n.id===e.source),b=state.graph.nodes.find(n=>n.id===e.target);if(!a||!b)continue;const isDirect=selected&&(e.source===selected||e.target===selected),dim=selected&&!isDirect&&!e.synthetic,relationColor=relationTypeById.get(e.type)?.color||'#567087';ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.strokeStyle=e.synthetic?(dim?'#1c2c39':'#2a4659'):(isDirect?relationColor:dim?'#172735':relationColor);ctx.globalAlpha=e.synthetic?.27:(isDirect?.95:(dim?.18:.52));ctx.lineWidth=(isDirect?2.3:e.synthetic?.65:1.15)/state.graph.scale;ctx.stroke();}
    ctx.globalAlpha=1;
    for(const n of state.graph.nodes){const sel=n.id===selected,dim=selected&&n.type==='knowledge'&&!sel&&!direct.has(n.id),r=n.r*(n.type==='domain'?1.12:1);ctx.beginPath();ctx.arc(n.x,n.y,r,0,Math.PI*2);ctx.fillStyle=color(n);ctx.globalAlpha=dim?.24:1;ctx.shadowColor=color(n);ctx.shadowBlur=(sel?24:n.type==='domain'?15:6)/state.graph.scale;ctx.fill();ctx.shadowBlur=0;if(sel){ctx.strokeStyle='#fff';ctx.lineWidth=2.5/state.graph.scale;ctx.stroke()}
      const showLabel=n.type==='domain'||sel||state.graph.scale>.82||(state.graph.scale>.55&&n.importance>=9);if(showLabel&&!dim){ctx.font=`${n.type==='domain'?'700 14':'600 11'}px Inter,system-ui`;ctx.textAlign='center';ctx.textBaseline='top';ctx.fillStyle='#eaf6ff';ctx.globalAlpha=.95;ctx.fillText(truncate(n.title,n.type==='domain'?26:30),n.x,n.y+r+7/state.graph.scale)}}
    ctx.restore();ctx.globalAlpha=1;
  }
  function fitGraph(){const ns=state.graph.nodes;if(!ns.length)return;const r=dom.stage.getBoundingClientRect(),minX=Math.min(...ns.map(n=>n.x-n.r)),maxX=Math.max(...ns.map(n=>n.x+n.r)),minY=Math.min(...ns.map(n=>n.y-n.r)),maxY=Math.max(...ns.map(n=>n.y+n.r)),s=Math.min((r.width-120)/(maxX-minX),(r.height-120)/(maxY-minY),1.25);state.graph.scale=Math.max(.18,s);state.graph.tx=r.width/2-((minX+maxX)/2)*s;state.graph.ty=r.height/2-((minY+maxY)/2)*s;drawGraph()}
  ...
  function updateDetails(n){
    const classification=classificationById.get(n.id)||{},simple=n.description?.simple||'',technical=n.description?.technical||'',architecture=n.description?.architecture||'',flags=auditFlags(n);dom.category.textContent=n.category;dom.category.style.borderColor=color(n);dom.title.textContent=n.title;dom.path.textContent=currentPath(n);
    dom.simpleSection.hidden=!simple;renderSemanticText(dom.short,simple,n.id);dom.technicalSection.hidden=!technical;renderSemanticText(dom.technical,technical,n.id);dom.architectureSection.hidden=!architecture;renderSemanticText(dom.architecture,architecture,n.id);dom.whySection.hidden=!n.why_important;renderSemanticText(dom.why,n.why_important||'',n.id);dom.examplesSection.hidden=!n.examples.length;renderSemanticText(dom.examples,n.examples.join('\n\n'),n.id);const memory=[n.merksatz&&`Merksatz: ${n.merksatz}`,n.analogy&&`Analogie: ${n.analogy}`].filter(Boolean);dom.memorySection.hidden=!memory.length;renderSemanticText(dom.memory,memory.join('\n\n'),n.id);dom.audit.hidden=!flags.length;dom.audit.textContent=flags.join('\n');
    dom.meta.innerHTML=`<span class="node-id-chip">ID: ${escapeHtml(n.id)}</span><span>${escapeHtml(classification.classification||'Canonical Node')}</span><span title="${escapeHtml(classification.proposed_target_position||'')}">Ziel: ${escapeHtml(truncate(classification.proposed_target_position||n.category,52))}</span><span title="${escapeHtml(confidenceLabel(classification.confidence))}">${escapeHtml(n.domain)}</span><span>${escapeHtml(n.metadata?.difficulty||'')}</span><span>Status: ${escapeHtml(n.metadata?.status||'')}</span>${(n.metadata?.certifications||[]).map(c=>`<span>${escapeHtml(c)}</span>`).join('')}`;
    ...
    const scenarioContexts=scenariosByNode.get(n.id)||[],stepContexts=stepsByNode.get(n.id)||[];
    dom.contextSection.hidden=!scenarioContexts.length&&!stepContexts.length;
    dom.contextList.innerHTML=[...scenarioContexts.map(s=>`<button class="context-link" data-scenario-id="${escapeHtml(s.id)}"><b>Architecture</b><br>${escapeHtml(s.title)}</button>`),...stepContexts.map(s=>`<button class="context-link" data-step-id="${escapeHtml(s.id)}"><b>Lernschritt</b><br>${escapeHtml(s.title)}</button>`)].join('');
    dom.contextList.querySelectorAll('[data-scenario-id]').forEach(b=>b.addEventListener('click',()=>openScenario(b.dataset.scenarioId)));
  ...
  function search(query){
    ...
    const results=scored.slice(0,30);dom.results.dataset.count=String(results.length);dom.results.innerHTML=results.map(({node:n,matchType,classification,currentPath:path})=>`<button data-id="${escapeHtml(n.id)}"><span class="search-result-heading"><b>${escapeHtml(n.title)}</b><em>${escapeHtml(matchType)}</em></span><small class="search-result-id">ID: ${escapeHtml(n.id)} · ${escapeHtml(classification.classification||'Canonical Node')}</small><small class="search-result-path">Aktueller Pfad: ${escapeHtml(path)}</small><small class="search-result-target">Zielbereich: ${escapeHtml(classification.proposed_target_position||n.category)}</small></button>`).join('')||'<button disabled>Keine Treffer</button>';
  ...
  dom.brain.addEventListener('pointerup',e=>{const p=pointInStage(e),hit=graphHit(p.x,p.y);if(hit&&hit.type==='knowledge'&&Math.abs(e.clientX-state.graph.last.x)<4&&Math.abs(e.clientY-state.graph.last.y)<4)selectNode(hit.id);state.graph.drag=false;state.graph.dragNode=null});dom.brain.addEventListener('pointerleave',()=>{state.graph.drag=false;dom.tooltip.hidden=true});
  ...
  dom.panel.addEventListener('click',event=>{const link=event.target.closest('.semantic-link');if(!link)return;const ids=link.dataset.targetIds.split(',').filter(Boolean);if(ids.length===1)navigateToNode(ids[0]);else{dom.search.value=link.dataset.term;search(link.dataset.term);dom.search.focus()}});
  dom.back.addEventListener('click',restoreNavigationContext);dom.close.addEventListener('click',clearDetails);dom.toggle.addEventListener('click',()=>{if(state.selected)toggleBranch(state.selected)});dom.showRelations.addEventListener('click',()=>{if(!state.selected)return;setMode('brain');buildGraph(state.selected);fitGraph()});
